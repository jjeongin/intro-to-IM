...
// Music 1: TOTORO
int totoro_melody[] = { // melody array referring to <pitch.h>
  NOTE_D5, NOTE_B4, NOTE_G4, NOTE_D5, NOTE_C5, NOTE_A4, 0, 0, 0
};

int totoro_durations[] = { // note durations: 4 = quarter note, 8 = eighth note, etc.
  2, 4, 2, 2, 2, 1, 4, 4, 4
};
...
void loop() {
  ...
  angle = map(angle, 0, 1023, 0, 270); // since potentiometer angle can rotate from 0 to 270 degree
  
  if (digitalRead(switchPin) == HIGH) { // if the switch is pushed
    if (angle <= 135) { // if potentiometer angle is smaller than 135, play totoro
    ...
    else { // if potentiometer angle is bigger than 135, play spirited away
